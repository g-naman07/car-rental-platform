...
const Navbar = () => {
    const {setShowLogin,user,logout,isOwner,axios,setIsOwner,toast} = useAppContext();
  ...
  location.pathname === "/" && "bg-light" }`}>
        ...
        <div className={`max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-16 max-sm:border-t border-borderColor right-0 flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-8 max-sm:p-4 duration-300 z-50 ${location.pathname === '/' ? "bg-light" : "bg-white"}
